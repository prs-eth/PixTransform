...
def PixTransform(source_img,guide_img,params=DEFAULT_PARAMS,target_img=None):
    ...
    #### setup network ############################################################################
    mynet = PixTransformNet(channels_in=guide_img.shape[0],weights_regularizer=params['weights_regularizer']).train().to(device)
            ...
            if epoch % params['logstep'] == 0:
                with torch.no_grad():
                    ...
                    if target_img is not None:
                        mse_predicted_target_img = F.mse_loss(source_img_std*predicted_target_img,source_img_std*target_img)
                    source_image_consistency = myloss(source_img_std*F.avg_pool2d(predicted_target_img.unsqueeze(0),D),source_img_std*source_img.unsqueeze(0))
                    tnr.set_postfix(MSE=mse_predicted_target_img.item(), consistency=source_image_consistency.item())
